...
var iNaturalistAPI = class iNaturalistAPI {
  static basicFetch( route, ids ) {
    ...
    return fetch( `http://${ iNaturalistAPI.apiHost }/${ route }/${ ids.join(",") }` ).
  ...
  static basicSearch( route, params ) {
    ...
    return fetch( `http://${ iNaturalistAPI.apiHost }/${ route }${ query }` ).
      then( iNaturalistAPI.thenCheckStatus ).
      then( iNaturalistAPI.thenText ).
      then( iNaturalistAPI.thenJson ).
      then( iNaturalistAPI.thenWrap );
  ...
  static setConfig( config ) {
    config = config | { };
    var envHostConfig = iNaturalistAPI.htmlDocumentHostConfig( ) ||
      iNaturalistAPI.nodeHostConfig( );
    iNaturalistAPI.apiHost = config.apiHost || envHostConfig || "localhost:4000";
    iNaturalistAPI.railsApiHost = config.railsApiHost || "localhost:3000";
  }

  static htmlDocumentHostConfig( ) {
    if( typeof document !== "undefined" &&
        typeof document.querySelector !== "undefined" ) {
      var element =
        document.querySelector('meta[name="config:inaturalist_api_host"]');
      return (element && element.getAttribute("content"));
    }
  }

  static nodeHostConfig( ) {
