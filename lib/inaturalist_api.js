...
var iNaturalistAPI = class iNaturalistAPI {
  ...
  static basicPost( route, params, options ) {
    ...
    if( route.match(/\/:id(\/|$)/) && (!params || !params.id) ) {
      var p1 = new Promise( function( res, rej ) {
        rej( new Error( "ID required" ) );
      });
      return p1;
    }
    var thisRoute = route.replace( ":id", params.id );
    var headers = {
  ...
  static methodHostPrefix( options ) {
    ...
    return `http://${ iNaturalistAPI.railsApiHost }`;
  }
  ...
  static basicUpdate( route, params, options ) {
    ...
    return iNaturalistAPI.basicPost( route, params, options );
  }
  ...
  static setConfig( config ) {
    ...
    iNaturalistAPI.railsApiHost =
      config.railsApiHost || envWriteHostConfig || "localhost:3000";
  }
